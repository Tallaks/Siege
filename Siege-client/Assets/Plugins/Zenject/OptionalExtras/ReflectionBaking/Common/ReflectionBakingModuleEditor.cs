...
using MethodBody = Zenject.ReflectionBaking.Mono.Cecil.Cil.MethodBody;
...
namespace Zenject.ReflectionBaking
{
	public class ReflectionBakingModuleEditor
	{
			...
			TypeDefinition typeDef, ReflectionTypeInfo typeInfo)
		{
			...
			MethodBody body = factoryMethod.Body;
			...
			List<MethodDefinition> propertySetMethods, List<MethodDefinition> postInjectMethods)
		{
			...
			MethodBody body = getInfoMethodDef.Body;
			...
			foreach (Instruction instruction in instructions) processor.Append(instruction);
		}
			...
			TypeReference specificTypeRef, string memberName)
		{
			...
			{
				PropertyDefinition candidatePropertyDef =
					typeRef.Resolve().Properties.Where(x => x.Name == memberName).SingleOrDefault();
			...
			TypeReference specificTypeRef, string fieldName)
		{
			foreach (TypeReference typeRef in specificTypeRef.GetSpecificBaseTypesAndSelf())
			{
				FieldDefinition candidateFieldDef = typeRef.Resolve().Fields.Where(x => x.Name == fieldName).SingleOrDefault();
			...
			out MethodReference methodRef)
		{
			foreach (TypeReference typeRef in specificTypeRef.GetSpecificBaseTypesAndSelf())
			{
				MethodDefinition candidateMethodDef =
					typeRef.Resolve().Methods.Where(x => x.Name == methodName).SingleOrDefault();
