...
namespace Zenject
{
	public class SignalBus : ILateDisposable
	{
		...
		public void LateDispose()
		{
			...
			{
				foreach (SignalSubscription subscription in _subscriptionMap.Values) subscription.Dispose();
			}

			foreach (SignalDeclaration declaration in _localDeclarationMap.Values) declaration.Dispose();
		}
		...
		public void DeclareSignal<T>(
			object identifier = null, SignalMissingHandlerResponses? missingHandlerResponse = null, bool? forceAsync = null,
		...
		private SignalDeclaration GetDeclaration(BindingId signalId)
		{
			...
			if (ParentBus != null) return ParentBus.GetDeclaration(signalId);
