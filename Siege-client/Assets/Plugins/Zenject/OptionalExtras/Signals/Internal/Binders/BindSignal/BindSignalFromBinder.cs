...
namespace Zenject
{
	public class BindSignalFromBinder<TObject, TSignal>
	{
		...
		public SignalCopyBinder From(Action<ConcreteBinderGeneric<TObject>> objectBindCallback)
		{
			...
			IfNotBoundBinder wrapperBinder = _container.Bind<IDisposable>().To<SignalCallbackWithLookupWrapper>().AsCached().
				WithArguments(_signalBindInfo, typeof(TObject), objectLookupId, methodGetterMapper).NonLazy();
