...
namespace Zenject
{
	public class BindSignalToBinder<TSignal>
	{
		...
		public SignalCopyBinder ToMethod(Action<TSignal> callback)
		{
			...
			BindInfo bindInfo = _container.Bind<IDisposable>().To<SignalCallbackWrapper>().AsCached()
				...
				.WithArguments(SignalBindInfo, (Action<object>)(o => callback((TSignal)o))).NonLazy().BindInfo;
