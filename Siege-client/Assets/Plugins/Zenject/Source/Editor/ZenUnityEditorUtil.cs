	...
	public static class ZenUnityEditorUtil
	{
		...
		public static void ValidateCurrentSceneSetup()
		{
			...
			try
			{
				...
				foreach (SceneContext sceneContext in GetAllSceneContexts()) sceneContext.Validate();
			}
			catch (Exception e)
			{
				Log.ErrorException(e);
			...
			if (encounteredError) throw new ZenjectException("Zenject Validation Failed!  See errors below for details.");
		}
		...
		{
			List<string> activeScenePaths = EditorBuildSettings.scenes.Where(x => x.enabled).Select(x => x.path).ToList();
		...
		public static SceneContext TryGetSceneContextForScene(Scene scene)
		{
			...
			List<SceneContext> sceneContexts =
				scene.GetRootGameObjects().SelectMany(x => x.GetComponentsInChildren<SceneContext>()).ToList();
