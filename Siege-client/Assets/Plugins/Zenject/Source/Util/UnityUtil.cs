...
namespace ModestTree.Util
{
	public static class UnityUtil
	{
		...
		public static IEnumerable<Transform> GetParents(Transform transform)
		{
			...
			foreach (Transform ancestor in GetParentsAndSelf(transform.parent)) yield return ancestor;
		}
		...
		{
			return gameObject.GetComponentsInChildren<Component>(includeInactive).OrderBy(x =>
		...
		{
			return gameObject.GetComponentsInChildren<Component>(includeInactive).OrderByDescending(x =>
				x == null ? int.MinValue : GetDepthLevel(x.transform));
		}
