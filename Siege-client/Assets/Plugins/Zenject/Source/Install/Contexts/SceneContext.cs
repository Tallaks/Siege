...
namespace Zenject
{
	public class SceneContext : RunnableContext
	{
		...
		private IEnumerable<DiContainer> GetParentContainers()
		{
			...
			List<DiContainer> parentContainers = UnityUtil.AllLoadedScenes.Except(gameObject.scene).
		...
		private List<SceneDecoratorContext> LookupDecoratorContexts()
		{
			...
			return UnityUtil.AllLoadedScenes.Except(gameObject.scene).SelectMany(scene => scene.GetRootGameObjects()).
				SelectMany(root => root.GetComponentsInChildren<SceneDecoratorContext>()).Where(decoratorContext =>
					_contractNames.Contains(decoratorContext.DecoratedContractName)).ToList();
		}
		...
		public void Install()
		{
			...
			foreach (MonoBehaviour instance in injectableMonoBehaviours) _container.QueueForInject(instance);

			foreach (SceneDecoratorContext decoratorContext in _decoratorContexts) decoratorContext.Initialize(_container);
		...
		private void InstallBindings(List<MonoBehaviour> injectableMonoBehaviours)
		{
			...
			_container.Bind(typeof(SceneKernel), typeof(MonoKernel)).To<SceneKernel>().FromNewComponentOn(gameObject).
				AsSingle().NonLazy();
