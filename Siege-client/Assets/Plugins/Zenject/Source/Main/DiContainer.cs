	...
	public class DiContainer : IInstantiator
	{
		...
		private object CreateLazyBinding(InjectContext context)
		{
			...
			object result = Activator.CreateInstance(
				typeof(LazyInject<>).MakeGenericType(newContext.MemberType), this, newContext);
					...
					try
					{
						...
						if (IsValidating)
							for (var i = 0; i < allInstances.Count; i++)
							{
								...
								if (instance is ValidationMarker) allInstances[i] = context.MemberType.GetDefaultValue();
							}
			...
			try
			{
				...
				if (matches.Count > 0)
					return matches.Select(
						x => x.Provider.GetInstanceType(context)).Where(x => x != null).ToList();
		...
		public FromBinderNonGeneric BindInterfacesTo(Type type)
		{
			...
			if (interfaces.Length == 0) Log.Warn("Called BindInterfacesTo for type {0} but no interfaces were found", type);
		...
		public CopyNonLazyBinder BindFixedTickableExecutionOrder(Type type, int order)
		{
			...
			return Bind<ValuePair<Type, int>>().WithId("Fixed").FromInstance(ValuePair.New(type, order)).
		...
		public CopyNonLazyBinder BindLateTickableExecutionOrder(Type type, int order)
		{
			...
			return Bind<ValuePair<Type, int>>().WithId("Late").FromInstance(ValuePair.New(type, order)).
				WhenInjectedInto<TickableManager>();
		}
