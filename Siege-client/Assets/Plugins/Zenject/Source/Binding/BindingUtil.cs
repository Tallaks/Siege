...
namespace Zenject
{
	internal static class BindingUtil
	{
		...
		{
			foreach (Type type in types) AssertIsNotComponent(type);
		}
		...
		{
			foreach (Type type in types) AssertDerivesFromUnityObject(type);
		}
		...
		{
			foreach (Type type in types) AssertIsInterfaceOrScriptableObject(type);
		}
		...
		{
			foreach (Type type in types) AssertIsInterfaceOrComponent(type);
		}
		...
		{
			foreach (Type type in types) AssertIsComponent(type);
		}
		...
		{
			foreach (Type type in types) AssertIsNotAbstract(type);
		}
		...
		{
			foreach (Type concreteType in concreteTypes) AssertIsDerivedFromTypes(concreteType, parentTypes);
		}
		...
		{
			foreach (Type parentType in parentTypes) AssertIsDerivedFromType(concreteType, parentType);
		}
