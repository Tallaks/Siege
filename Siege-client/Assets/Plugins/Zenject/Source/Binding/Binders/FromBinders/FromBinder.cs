...
namespace Zenject
{
	public abstract class FromBinder : ScopeConcreteIdArgConditionCopyNonLazyBinder
	{
			...
			Action<ConcreteBinderGeneric<IFactory<TContract>>> factoryBindGenerator)
		{
			...
			ConcreteBinderGeneric<IFactory<TContract>> subBinder =
				BindContainer.BindNoFlush<IFactory<TContract>>().WithId(factoryId);
				...
				(container, concreteType) => new MethodMultipleProviderUntyped(ctx =>
					{
						...
						return monoBehaviour.GetComponents(concreteType).Where(x => !ReferenceEquals(x, monoBehaviour));
					},
					container));
